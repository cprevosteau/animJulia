...
//////////////////////GPU PROGRAM
	...
	/////Déclaration  des objets qui seront stockées dans la mémoire constante du GPU
		...
		__constant__ double param[5];					// Paramètres arguments de la génération des ensembles de Julia :
...
void gpRaff(double* t_param,unsigned char* t_couDes,unsigned char* t_couFond,sf::Time tdif)
{
	int vitesseD(500);			// vitesse de déplacement de l'image pour x et y
	double vitesseZoom(1.5);	// vitesse de zoom ou dézoom
	int vitesseC(10);			// vitesse de changement de la constante complexe
	int vitesseCou(50);			// vitesse de changement de couleur
	...
	//Modification de l'échelle
			...
			{
				t_param[4]/=pow(vitesseZoom,tdif.asSeconds());
			}
...
__device__ double gpJuCouleur(double const& x, double const& y)
{
	double zr(x);
	double zi(y);
	double zr0;
...
__global__ void dessinKernel(sf::Uint8* dev_pTabPix)
{
	...
	{
			double x(param[2]+(i-(double)(dim[0])/2.)*(param[4]));  // On associe les coordonnées d'un complexe au pixel
			double y(param[3]+(j-(double)(dim[1])/2.)*(param[4]));  // selon les paramètres.

			double pui(gpJuCouleur(x,y));							// On récupère l'intensité associé à ce complexe.
...
void gpDessiner(sf::Uint8* pTabPix,sf::Uint8* dev_pixels,double *cpu_param,unsigned char *cpu_couDes,unsigned char *cpu_couFond, unsigned int* cpu_dim)
{
	...
	//// Mise à jour des variables constantes (sur le GPU) à partir de leurs homologues hôtes
		...
		cudaMemcpyToSymbol(param, cpu_param, 5*sizeof(double));
...
int main()
{
	...
	// Initiallisation
		...
		double* t_param =new double[5];				// Initiallisation du vecteur de paramètres
		t_param[0]=-0.7927;
		t_param[1]=0.1609;
		t_param[2]=0;
		t_param[3]=0;
